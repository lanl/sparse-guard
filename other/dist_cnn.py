...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        plt.draw()
        plt.savefig(f'./distribution/img/cnn/img{batch}.jpg', dpi=100, bbox_inches='tight')
...
def plot_dist():
    ...
    for image in os.listdir('./distribution/img/cnn/'):
        img = Image.open('./distribution/img/cnn/'+image)
        x = np.array(img)
        x = x.transpose(2, 0, 1)
    ...
    plt.savefig(f'./distribution/cnn_dist.jpg', dpi=100, bbox_inches='tight')
