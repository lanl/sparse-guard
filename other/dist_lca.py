...
class SplitNN(nn.Module):
  ...
  def forward(self, x):
    ...
    return x
'''    
target_model = SplitNN().to(device=device, dtype=torch.float32)
...
def attack_test(train_loader, target_model, attack_model):
    ...
    for batch, (data, targets) in enumerate(tqdm(train_loader)):
        ...
        DataI=target_outputs[:,:, 0:3]
        #img= torch.permute(DataI, (2, 1,0))
        #img=img.to(torch.float32)
        #print(img.shape)
        plt.imshow((DataI.cpu().detach().numpy()))
        plt.xticks([])
        plt.yticks([])

        plt.draw()
        plt.savefig(f'./distribution/img/lca/img{batch}.jpg', dpi=100, bbox_inches='tight')
...
def plot_dist():
    ...
    for image in os.listdir('./distribution/img/lca/'):
        img = Image.open('./distribution/img/lca/'+image)
        x = np.array(img)
        x = x.transpose(2, 0, 1)
    ...
    plt.savefig(f'./distribution/lca_dist.jpg', dpi=100, bbox_inches='tight')
